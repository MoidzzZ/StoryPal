...
- 本轮定向回归：Python 进度／手账／Note／故事边界 **31/31 通过**，WebUI 阅读器与用户标识 **2/2 通过**；本地服务入口和健康接口均为 200，未认证阅读接口为 401。结果细节见根目录 `result.md`。
- 实际网页点击、划选续读及真实 Luna 对话未通过自动化验收，仍待核验；下一步先补 R2/R3 前端行为覆盖，并扩大自动 Note 的中文难例。
