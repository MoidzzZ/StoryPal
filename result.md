...
## 13. 首次阅读小规模自动验收（2026-09-24）

- 验收场景与通过标准见 [首次阅读行为验收规范](docs/operations/FIRST_READING_ACCEPTANCE.md)。模拟读者由 GPT-5.6 Terra 承担，限定为 7 个场景；未调用 Luna 或修改真实用户状态。
- Conda `storypal-chatbot`、项目内临时目录：阅读器、进度／手账、Note 与故事边界的 Python 定向回归 **31/31 通过**（分两批为 21/21、10/10）。WebUI 阅读器和用户标识组件 **2/2 通过**。
- 本地 `/health` 与首页均为 **200**，未认证阅读接口为 **401**。没有请求或输出认证后的小说原文。
- 这些证明程序契约和服务入口可用；浏览器真实点击、划选和续读，以及真实 Luna 对话、回答质量、确认措辞均尚未验收。浏览器自动化通道本轮异常，不能把测试通过写成真人体验通过。

## 14. WebUI 人格文档和阅读入口修复（2026-09-24）

- 运行时差异：原 Conda editable 安装指向 `.reference/nanobot`，而人格文档接口只在 `.runtime/nanobot/source-build`。切换前该运行包无法导入 `_persona_documents`；切换后本机配置下三份文档均可读。
- 合并工具白名单补丁后的候选源码，后端定向回归 **110/110**；人格页、阅读器和整体布局前端定向回归 **62/62**；TypeScript 检查与生产构建通过。阅读器补丁重放检查通过。
- 重启后的受保护设置 API 返回 `SOUL.md`、`AGENTS.md`、`USER.md` 三项且均 `available=true`；首页 **200**，加载新构建资源。只检查可用标志和长度，不输出正文、密钥或原文。
- 阅读入口已改为独立工具栏，不再绝对覆盖聊天控件。浏览器自动化通道仍无法初始化，尚未完成实际视觉验收；需用户刷新页面后确认无重叠、三个文档可切换阅读。
